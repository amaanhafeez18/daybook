...
describe('clampEstimateItem', () => {
  ...
  test('clamps and rounding', () => {
    ...
    assert.deepEqual(item.extra, { alcoholG: 3.3, caffeineMg: 96, satFatG: null })
    assert.equal(clampEstimateItem({ name: 'Butter', sat_fat_g: 700.26 }).extra.satFatG, 500)
    assert.equal(clampEstimateItem({ name: 'Butter', extra: { satFatG: 7.26 } }).extra.satFatG, 7.3)
