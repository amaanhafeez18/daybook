...
`supabase/schema.sql` for a fresh DB, then every file in `supabase/migrations/` by date. The notifications one needs `YOUR_CRON_SECRET` replaced. Possibly still pending for the owner (verify before assuming): the `patch_settings` function from `2026-09-26-gym.sql` (its tables exist; the function was missing as of 2026-09-24), `2026-09-25-cron-log-cleanup.sql`, `2026-09-28-tasks-completed-at.sql` (`tasks.completed_at`; the app and assistant tolerate its absence). The notifications tables exist. Check what exists with a read-only query using the local secret key if needed.
