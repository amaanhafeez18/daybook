...
import { MealSelect } from './EstimateReview.jsx'
...
// estimate), a photo, or a voice note. Focusing it opens a tray above with Suggested · Recent ·
// Favorites (one tap logs, with Undo), a meal picker, and links to manual entry / quick calories.
// While typing, the tray shows matching foods you've logged before.
...
export default function QuickAddBar({ date, today, meal, mealChosen = false, onMealChange, entries, food, controlRef, onEstimate, onManual }) {
  ...
  const lists = useMemo(() => {
    ...
    if (query) {
      ...
      for (const item of [...favorites, ...recent]) {
        ...
        const haystack = [item.name, item.brand, ...(item.aliases || [])].filter(Boolean).join(' ').toLowerCase()
  ...
  }, [trayOpen, entries, food.favorites, meal, date, today, query])
  ...
  function submit(event) {
    ...
    if (!words) return
    onEstimate({ text: words, source: 'ai_text' })
    setText('')
    setTrayOpen(false)
    input.current?.blur()
  }
  ...
  async function onPhoto(event) {
    ...
    try {
      ...
      onEstimate({ image, text: text.trim() || undefined, source: 'ai_photo' })
      setText('')
      setTrayOpen(false)
    } catch (error) {
  ...
  const emptyText = {
    ...
    favorites: 'Tap the star when adding a food to keep it here.',
  }[tab]
      ...
      {trayOpen && (
        <div className="food-tray" role="dialog" aria-label="Quick add">
          ...
          <ul className="food-tray-list">
            {query && (
              <li>
                <button type="button" className="food-tray-row is-ai" onClick={submit}>
                  <span className="food-tray-icon"><Icon name="sparkles" size={17} /></span>
                  <span className="food-tray-text">
                    <span className="food-tray-name">Estimate “{text.trim()}”</span>
                    <span className="food-tray-sub">AI estimate you can review</span>
                  </span>
            ...
            {query && list.length === 0 && <li className="food-tray-empty">Nothing logged before matches — tap above to estimate it.</li>}
          </ul>
        </div>
      )}
      ...
      {recorder.recording ? (
        ...
        <form className="food-bar" onSubmit={submit}>
          ...
          <input
            ...
            placeholder={mealChosen ? `Add to ${mealName(meals, meal).toLowerCase()}…` : 'Describe food…'}
            aria-label="Describe what you ate"
            enterKeyHint="send"
            autoComplete="off"
            maxLength={1000}
          />
          <button type="button" className="food-bar-btn" onClick={() => fileRef.current?.click()} disabled={preparing} aria-label="Add a photo of your food">
            {preparing ? <span className="spinner" aria-hidden="true" /> : <Icon name="camera" size={21} />}
          </button>
          {text.trim() ? (
