...
// #/food/goals: "Calculate for me" (profile → Mifflin-St Jeor / Katch-McArdle maintenance, a
// pace, safety floors, then editable macros) or "Set manually". Every goal is optional.
...
export default function GoalWizard({ today }) {
  ...
  const [moreOpen, setMoreOpen] = useState(() => isNum(food.profile.bodyFatPct))
  ...
  useEffect(() => {
    ...
    setMoreOpen(isNum(food.profile.bodyFatPct))
  ...
  const rateLabel = (kg) => (weightUnit === 'lb' ? `${LB_LABELS[kg] ?? fmtNum(kg / 0.45359237, 2)} lb` : `${fmtNum(kg, 2)} kg`)
  ...
  const header = (
    <>
      ...
      <header className="food-page-head" ref={topRef}>
        <h1>Goals</h1>
        <p className="page-subtitle">Every goal is optional. Change them any time.</p>
      </header>
    </>
  )
  ...
  return (
    <div className="food-goals">
      {header}
      <Segmented
        options={[{ id: 'calc', label: 'Calculate for me' }, { id: 'manual', label: 'Set manually' }]}
        value={mode}
        onChange={(value) => { setMode(value); setStep(1) }}
        label="How to set goals"
        className="food-goals-mode"
      />
        ...
        <form className="food-cfg" onSubmit={calculate}>
          <section className="food-cfg-group">
            ...
            <div className="food-cfg-card">
              <div className="food-cfg-row is-stacked">
                <span className="food-cfg-label"><span>Sex</span><small>Used for the energy equation — optional</small></span>
              ...
              <div className="food-cfg-row is-stacked">
                <span className="food-cfg-label"><span>Current weight</span><small>{isNum(latestKg) ? 'From your weight log' : 'Also saved to your weight log'}</small></span>
          ...
          <section className="food-cfg-group">
            ...
            <div className="food-cfg-card">
              ...
              {rates.length > 0 && (
                <div className="food-cfg-row is-stacked">
                  <span className="food-cfg-label"><span>Pace per week</span></span>
          ...
          {moreOpen ? (
            <section className="food-cfg-group">
              <h3 className="food-cfg-title">More</h3>
              <div className="food-cfg-card">
                <div className="food-cfg-row is-stacked">
                  <span className="food-cfg-label"><span>Body fat</span><small>Optional — if you know it, the estimate uses lean mass instead</small></span>
                  <UnitInput value={form.bodyFat} onChange={(value) => set('bodyFat', value)} suffix="%" label="Body fat percent" placeholder="—" />
                </div>
              </div>
            </section>
          ) : (
            <button type="button" className="food-es-disclose food-goals-more" onClick={() => setMoreOpen(true)}>More options<Icon name="chevronRight" size={16} /></button>
          )}
...
function UnitInput({ value, onChange, suffix, label, placeholder, inputMode = 'decimal' }) {
  return (
    <span className="food-unit-input">
      <input className="input" inputMode={inputMode} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} aria-label={label} autoComplete="off" enterKeyHint="done" />
...
function GoalResult({ result, profile, unit, weightUnit, today, macros, setMacros, onBack, onSave }) {
  ...
  const paceText = isNum(pace) && Math.abs(pace) >= 0.01
    ? `${pace < 0 ? 'Losing' : 'Gaining'} about ${fmtNum(Math.abs(kgToUnit(pace, weightUnit)), 2)} ${u} a week`
    ...
    <div className="food-cfg food-goal-result">
      ...
      <section className="card food-goal-hero">
        ...
        <span className="food-goal-hero-value">
          {/* As wide as its digits: a 5-digit kJ goal (10,878) must not be cut off. */}
          <input
            className="food-goal-hero-input"
            inputMode="numeric"
            value={macros.calories}
            onChange={(event) => set('calories', event.target.value.replace(/[^\d]/g, '').slice(0, 5))}
            aria-label={`Daily calories in ${unitLabel(unit)}`}
            style={{ width: `${Math.max(3, String(macros.calories || '').length) + 0.6}ch` }}
          />
        ...
        <dl className="food-goal-facts">
          <div><dt>Maintenance</dt><dd>{fmtInt(energyInUnit(result.tdee, unit))}</dd></div>
          <div><dt>At rest (BMR)</dt><dd>{fmtInt(energyInUnit(result.bmr, unit))}</dd></div>
        </dl>
      </section>
      ...
      <section className="food-cfg-group">
        <h3 className="food-cfg-title">Macros a day</h3>
        <div className="food-cfg-card">
        ...
        <p className="food-cfg-foot">
          {mismatch
            ? `Protein, carbs and fat add up to ${fmtInt(energyInUnit(macroKcal, unit))} ${unitLabel(unit)} — ${macroKcal > kcal ? 'more' : 'less'} than the calorie goal.`
            : `Protein ${fmtNum(result.refKg ? result.protein / result.refKg : 0, 1)} g per kg; fat about ${Math.round(((result.fat * 9) / result.calories) * 100)}% of calories; carbs make up the rest.`}
        </p>
      </section>
          ...
          {keys.map((key) => {
            ...
            return (
              <div key={key} className={`food-cfg-row${info ? ` food-goal-macro is-${key}` : ''}`}>
                ...
                <span className="food-cfg-control">
                  <UnitInput value={values[key]} onChange={(value) => setValues((current) => ({ ...current, [key]: value }))} suffix={suffix} label={`${label} goal`} placeholder="—" />
                </span>
              </div>
            )
          })}
